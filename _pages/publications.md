...
<div class="paper">
  ...
  <button onclick="toggleAbstract(this)">Show Abstract</button>
  <div class="abstract" style="display:none; margin-top:10px;">
    <p>The rapid growth of online shopping has made consumer search data increasingly available, offering new opportunities for empirical research. Sequential search models provide a structured framework for analyzing such data, but their application is often hindered by technical challenges. In particular, existing methods rely on step-by-step optimal search rules, which depend on unobserved search outcomes and lead to high-dimensional likelihood functions that are difficult for estimation. This paper proposes a new approach by showing that the optimal decision-making in a broad class of sequential search models can be equivalently represented as a partial ranking over all actions available during the search process. This ranking-based representation avoids complications from unobserved outcomes and enables more tractable analysis. Building on this insight, we derive a decomposed likelihood function, clarify identification arguments, and propose an improved GHK-style estimator that is both simpler to implement and more robust in performance. We further extend this method to handle common extensive empirical settings like incomplete search data and richer models that include additional actions, such as product discovery. Our approach offers a novel perspective on sequential search and provides a practical toolkit for researchers and practitioners working with search data.</p>
